...
const DEFAULTS = {
  ...
  articles: [
    {id:1,title:"為什麼你記帳卻還是存不到錢？",date:"2026-05-01",tag:"理財觀念",excerpt:"很多人以為記帳就能存到錢，但其實記帳只是照妖鏡，照出你花了哪些錢，卻不會自動幫你存錢。真正的關鍵是——先存後花。",content:"很多人問我：「Barbara，我有記帳，但怎麼還是存不到錢？」\n\n這個問題我自己以前也有過。記帳本寫得密密麻麻，每個月花了多少一清二楚，但帳戶餘額還是在月底趨近於零。\n\n後來我才發現：記帳只是照妖鏡，不是存錢器。\n\n它能告訴你錢去哪了，但它不會幫你把錢留下來。\n\n真正讓我開始存到錢的方法，是把順序倒過來：先存，再花。\n\n發薪日當天，先把要存的錢轉出去——不管是緊急備用金、旅遊基金、還是投資帳戶。剩下的才是這個月可以動用的生活費。\n\n從「記帳→花錢→希望有剩」，變成「存錢→花錢→用記帳確認」。\n\n這個順序的改變，對我來說比任何記帳軟體都有效。",views:312,comments:[{name:"8友小美",text:"這個觀念真的打到我了，我也是每個月記帳但存不到錢",date:"2026-05-03"},{name:"匿名",text:"先存後花，我試了兩個月真的有效！謝謝Barbara",date:"2026-05-05"}]},
    {id:2,title:"信用卡不是壞東西，是你沒搞清楚規則",date:"2026-05-15",tag:"信用卡",excerpt:"很多人怕信用卡，覺得它會讓自己亂花錢。但其實信用卡是中性工具，問題在於你有沒有把帳單日和繳款日搞清楚。",content:"「我不敢辦信用卡，怕自己亂花錢。」\n\n這句話我聽過很多次。信用卡在很多人眼中是個危險物品，好像辦了就會失控。\n\n但我想說的是：信用卡本身是中性的，問題從來都不是卡，是使用的方式。\n\n真正讓人「失控」的原因：一、不知道自己實際花了多少。二、搞不清楚帳單日和繳款日的差異。\n\n很多人以為「繳款日前只要繳清就好」，卻不知道帳單日到繳款日之間還有一段緩衝期，這段時間如果繼續刷，會累積到下一期帳單。\n\n我的建議：把信用卡的預算視同現金在管理。刷了就登記，不要等帳單才知道。設定自動扣款，永遠不繳最低應繳。\n\n關鍵從來不是「辦不辦卡」，是「你有沒有在掌控它」。",views:198,comments:[{name:"匿名",text:"終於有人說清楚帳單日跟繳款日的差別了！",date:"2026-05-17"}]}
  ],
  products: [
...
function Article({ article, onBack, setArticles, isAdmin, tags }) {
  ...
  return (
    <div style={{ maxWidth:740, margin:"0 auto", padding:"48px 20px" }} className="page-pad">
      ...
      <div style={{ background:O2, padding:"24px", marginBottom:44, borderLeft:`3px solid ${O}` }}>
...
export default function App() {
  const [articles, setArticles, aLoaded] = useFirestore("articles", DEFAULTS.articles);
  const [products, setProducts, pLoaded] = useFirestore("products", DEFAULTS.products);
  const [igPosts, setIgPosts, iLoaded] = useFirestore("igPosts", DEFAULTS.igPosts);
  const [goods, setGoods, gLoaded] = useFirestore("goods", []);
  ...
  if (!loaded) return (
    <>
      ...
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <p style={{ fontSize:13, color:LIGHT, letterSpacing:"2px" }}>載入中⋯</p>
      </div>
    </>
  );
